...
在目标检测任务中，模型会识别出许多可能包含目标的区域，这些区域被称为候选区域。每个区域都会有一个置信度得分，表示模型认为该区域包含目标的可能性。因为这些区域往往会有重叠，我们需要一种方法来确定哪些区域最有可能真正包含目标。

非极大值抑制的步骤如下：

	1.	对于某一类别的所有区域，根据它们的置信度得分进行排序。
	2.	选择置信度得分最高的区域，并移除所有与它重叠较大（即交并比（IoU）高于某个阈值τ）的区域。
	3.	重复第2步，直到所有区域都被处理。

交并比（IoU）（Intersect over Union）是一个衡量两个区域重叠程度的指标，计算为两个区域的交集面积除以它们的并集面积。

通过这个过程，NMS确保每个检测到的对象只被选择一次，并且是置信度得分最高的一个。这是一个在目标检测算法中常见的后处理步骤，用于提高检测的准确性。



#### Two edge detection 

<a href="https://sm.ms/image/JVeU3E1MkAqFwp2" target="_blank"><img src="https://s2.loli.net/2024/04/09/JVeU3E1MkAqFwp2.png" ></a>
...
$$
\text { loss }=\left(P_x-G_x\right)^2+\left(P_y-G_y\right)^2+\left(P_w-G_w\right)^2+\left(P_h-G_h\right)^2
$$

Instead of learn the $x y$ w $h$ value, learn the offset ratio
$$
W_*=\operatorname{argmin}_{\bar{W}_*} \sum_i^N\left(t_*^i-\bar{W}_*^T \phi_5\left(P^i\right)\right)^2+\lambda\left\|\bar{W}_*\right\|^2
$$
$$
\begin{aligned}
...
$$
	...
	•	R-CNN：是最初的版本，使用选择性搜索（SS）来提出区域，然后利用卷积神经网络（CNN）提取特征，使用支持向量机（SVMs）来分类，以及回归来细化区域。
	•	Fast R-CNN：对R-CNN的改进，它在特征提取后直接使用Softmax进行分类，并且同时进行区域细化，整体上更快。
	•	Faster R-CNN：在Fast R-CNN的基础上进一步加速，通过引入区域建议网络（RPN），与Fast R-CNN共享卷积特征，大大减少了计算时间。

在表格中，你可以看到测试时间从R-CNN的50秒减少到Faster R-CNN的0.2秒，速度提升从1倍到250倍，而在VOC 2007数据集上的mAP几乎保持不变，在66.0到66.9之间。



bonding box的长宽位置，revise的过程是一个regression 的过程

<a href="https://sm.ms/image/uyzXQFrB3k8VsSi" target="_blank"><img src="https://s2.loli.net/2024/04/09/uyzXQFrB3k8VsSi.png" ></a>



<a href="https://sm.ms/image/RBxcLAwaVJjUvpm" target="_blank"><img src="https://s2.loli.net/2024/04/09/RBxcLAwaVJjUvpm.png" ></a>

bonding box 可能就是一个background

#### Faster R-CNN（一种目标检测模型）的优势和劣势。

优势：

	•	速度：Faster R-CNN能以0.2秒每张图像的速度进行检测，包括提出区域的提取，比Fast R-CNN快10倍。这是因为它使用了更少的层来构建区域建议网络（RPN）和对特定区域进行分类及回归。
	•	端到端训练：可以连续训练模型的所有部分，包括特征提取、区域提案、分类器和回归器，这样可以提高整体性能和准确性。
	•	准确性：区域提案是通过学习得到的，而且RPN是卷积的，这使得模型能够更准确地预测对象的位置。

劣势：

	•	全连接层：尽管提速，Faster R-CNN仍需用于特定区域任务的多个全连接层，这可能增加了计算负担。
	•	单尺度：它还是一个单尺度的网络，意味着它可能不擅长处理不同大小的图像或目标。

<a href="https://sm.ms/image/JswL7dPlp9qG5Rv" target="_blank"><img src="https://s2.loli.net/2024/04/09/JswL7dPlp9qG5Rv.png" ></a>

# Lec 11 Obiject detection

#### 单次检测器(single shot detector)



<a href="https://sm.ms/image/B6bVjSfq1I5cYnM" target="_blank"><img src="https://s2.loli.net/2024/04/11/B6bVjSfq1I5cYnM.png" ></a>



优点：

	•	最佳权衡：SSD500的速度为每秒处理23帧，SSD300为每秒处理58帧，性能上接近或优于更快的R-CNN，比YOLO更好。
	•	在不增加额外成本的情况下，可以处理多个尺度：与YOLO相比，能进行更多检测，且不需要RoI池化。
	•	边界框回归是卷积的，像区域提议网络（RPN）一样，但具有更简单的流程，并且在相同数量的锚点下有更多的宽高比。

缺点：

	•	金字塔从低分辨率开始：难以处理小物体。

特征金字塔网络（Feature Pyramid Network, FPN）和残差网络（ResNet）来检测图像中的对象。

<a href="https://sm.ms/image/2UKBDEMAX9Se4fH" target="_blank"><img src="https://s2.loli.net/2024/04/11/2UKBDEMAX9Se4fH.png" ></a>



	•	基础网络（Base Network）：使用的是ResNet-101，这是一个具有101层的深度残差网络，用于提取图像特征。
	•	特征金字塔网络（Feature Pyramid Network）：构建在基础网络之上，通过融合不同层次的特征（C2到C5），在多个尺度上进行目标检测。
	•	多尺度密集检测（Multi-scale Dense Detection）：使用来自特征金字塔的多尺度特征（P2到P5）进行对象的检测，每个尺度都用于检测不同大小的对象。

每个“det(c, a)”表示一个检测头，它对特定尺度的特征图（例如P2, P3, P4, P5）进行分类和边界框回归。在这里，“c”通常表示类别的数量，而“a”表示每个位置的锚点数量。这样的设计使RetinaNet能够有效地在图像的不同区域和尺度上进行准确的目标检测。



	•	分类分数（c classification scores）：对于每个锚点 （在此 ，代表每个位置的9个锚点，由3种尺度和3种宽高比组成），模型都会预测  个分类分数。
	•	边界框坐标（4 bounding box coordinates）：为每个锚点预测4个与之相关的边界框坐标。
	•	Focal Loss：用于处理分类得分，这是一种特殊的损失函数，旨在解决正负样本数量极度不平衡的问题。同时，坐标回归也使用回归损失函数。
	•	参数独立：分类和回归分支之间不共享参数。
	•	检测子网参数共享：检测子网的参数在所有金字塔层级之间共享。

简而言之，RetinaNet的这部分设计旨在高效地进行多尺度和多比例的对象检测，同时处理类别不平衡的问题。通过每个锚点的分类和回归预测，模型能够识别和定位图像中的不同对象。

classification dectection segementation

<a href="https://sm.ms/image/nh47cXO1v8kTI9M" target="_blank"><img src="https://s2.loli.net/2024/04/11/nh47cXO1v8kTI9M.png" ></a>

1.	选择核函数和带宽：核函数定义了样本点周围区域的权重，带宽决定了这个区域的大小。
	2.	对每个点执行以下步骤：
	   a. 在该点上中心化一个窗口。
	   b. 计算搜索窗口中数据的平均值。
	   c. 将搜索窗口移至新的平均值位置。
	   d. 重复步骤b和c直到收敛，即搜索窗口的中心不再显著移动。
	3.	分配聚类：将导致附近模式的点分配给同一个聚类。
	
	<a href="https://sm.ms/image/IPFu8cEG9SQvoka" target="_blank"><img src="https://s2.loli.net/2024/04/11/IPFu8cEG9SQvoka.png" ></a>
	
	## Lec 11 Cycle GAN
	
	<a href="https://sm.ms/image/TDXk9Ct6hod3IlW" target="_blank"><img src="https://s2.loli.net/2024/04/16/TDXk9Ct6hod3IlW.png" ></a>
	
	

左边有一个标题写着“Paired”，下面是一系列配对的图像。每一对图像包含一个轮廓图（标记为 Xi*X**i*）和一个对应的彩色图像（标记为 Yi*Y**i*）。这些图像配对通常用于监督学习，尤其是在图像分割和像素级标注中。在这里，模型会学习如何从轮廓图生成对应的详细彩色图像。

中间的部分展示了从标签图像到照片的转换，被称作“Label ↔ photo: per-pixel labeling”。标签图像是一个分割图，其中每个像素都被标记为属于特定类别（比如车辆、树木等），而目标是生成一个真实世界的照片风格的图像。

最后，右边有一个疑问标记，下面有文字“Horse ↔ zebra: how to get zebras?”，意味着这是一个问题：如何从马的图片生成斑马的图片。这代表了一种图像到图像的转换任务，通常是在没有成对训练数据的情况下完成的。下面的文字说明了这个问题的难点：“Expensive to collect pairs.”（收集成对数据昂贵）和“Impossible in many scenarios.”（在许多情况下不可能实现）。这可能是在讨论一种无监督学习方法，如循环一致性对抗网络（CycleGAN），它能够在没有成对训练样本的情况下转换图像的风格。



- **生成器 (G)**：其作用是创建数据。在这个例子中，它接收一张马的图片作为输入（表示为 X*X*），然后生成一张新的图片（表示为 G(x)*G*(*x*)），看起来像是斑马的图片。
- **鉴别器 (D)**：它的作用是评估生成的图片和真实图片，尝试区分出哪些是“假”的（即由生成器创建的），哪些是“真”的（即实际的斑马图片）。在这个过程中，鉴别器被训练来识别生成器产生的图像，并且在图中的文字“Real too!”暗示着鉴别器有时也会被生成器欺骗，错误地将生成的图像识别为真实图像。



<a href="https://sm.ms/image/NuWKdXM3qCLTZ1s" target="_blank"><img src="https://s2.loli.net/2024/04/16/NuWKdXM3qCLTZ1s.png" ></a>

**基于文本的图像检索**：

- 这是一种早期的方法，可以追溯到1970年代。
- 在这种系统中，图像由人工添加文本描述符来进行注解。
- 这些描述符然后被数据库管理系统使用来检索图像。
- 此方法的缺点包括：需要大量的人工劳动来手工注解图像，以及由于人类感知的主观性，注解可能不准确。

**基于内容的图像检索 (CBIR)**：

- 为了克服基于文本检索的缺点，CBIR在1980年代初被引入。
- 在CBIR中，图像通过它们的视觉内容来索引，例如颜色、纹理、形状等。

图片的右侧展示了CBIR系统的一个流程：

1. **查询图像**：你有一个查询图像，并希望找到类似的图像。
2. **特征提取**：从查询图像中提取特征。
3. **计算最接近的特征并按距离排序**：计算查询图像的特征与数据库中每个图像的特征的相似度。
4. **获取相应图像**：从图像数据库中获取特征距离最近的图像。
5. **返回排序后的图像**：将这些图像返回给用户。

图中也有一个部分展示了一个简化的CBIR系统架构，包括图像数据库、特征提取和存储特征在数据库中的步骤。这表明CBIR系统如何工作，通过自动化的特征提取和比较过程，避免了基于文本的系统中的人工注解工作。

大多数图像和视频的集合并不构成真正的图像数据库。这里的两个主要问题是：

1. **没有数据库管理系统(DBMS)来管理数据**：这意味着图像和视频通常没有被整合到一个可以高效管理和查询的系统中。
2. **没有复杂查询的设施**：当前的系统不支持复杂的、基于图像内容的查询。

幻灯片下半部分提出了一系列重要的研究问题，这些问题说明了计算机视觉领域的活跃研究方向和当前的技术需求：

1. 如何找到我们感兴趣的特定图像？
2. 我们能定位到满足某些描述的图像吗？
3. 我们如何有意义地组织收集到的图像？
4. 我们能否通过自动探索收集来自动提取新信息？

颜色直方图的工作原理如下：

- **颜色空间离散化**：将RGB颜色空间（或其他颜色空间）离散化为多个“箱子”（bins）。离散化意味着将连续的颜色空间分割成有限数量的小区间，每个小区间代表一种颜色范围。
- **像素计数**：对于每个箱子，统计落入该箱子的像素数量。每个像素根据其颜色被分类到相应的箱子中。

一旦构建了颜色直方图，就可以使用多种度量方法来比较不同图像的直方图。比如，可以通过计算两个直方图之间的相似度或距离来判断两幅图像在颜色分布上的相似程度。

- **欧氏距离**：计算两个直方图各个相对应箱子的值之间差的平方和的平方根。
- **卡方测试**：评估两个直方图的统计显著性差异。
- **巴氏距离**：衡量两个概率分布的重叠程度。
- **交叉熵**：度量一个概率分布在另一个概率分布下的期望。

<a href="https://sm.ms/image/7mHbGzNAvqfixJY" target="_blank"><img src="https://s2.loli.net/2024/04/16/7mHbGzNAvqfixJY.png" ></a>

<a href="https://sm.ms/image/7mHbGzNAvqfixJY" target="_blank"><img src="https://s2.loli.net/2024/04/16/7mHbGzNAvqfixJY.png" ></a>

对比损失（Contrastive Loss）：
- $L_{\text {contrastive }}$ 表示对比损失函数。
- 它由两部分组成：当两个样本属于同一类别 $(y=0)$ 时, 它们之间的距离 $D\left(x_i, x_j\right)$ 应该是小的（这部分被称为"Pull"）; 而当它们属于不同类别 $(y=1)$ 时, 距离应该大于一个边界值 $m$ （这部分被称为"Push"）。
- 公式 $(1-y) \times D\left(x_i, x_j\right)+y \times \max \left(0, m-D\left(x_i, x_j\right)\right)$ 体现了这两个直观的想法：同类靠近, 异类远离。

三元组损失（Triplet Loss）：
- $L_{\text {triplet }}$ 表示三元组损失函数。
- 它侧重于将一个参考样本（针点 a）与一个正样本 (p) 之间的距离与该参考样本与一个负样本 (n)之间的距离进行比较。
- 三元组损失确保针点与正样本之间的距离小于针点与负样本之间的距离至少一个边界 $m$ 。
- 公式 $\max (0, D(a, p)-D(a, n)+m)$ 用于实现这一目标。

在这两种损失函数中， $D$ 表示距离度量（通常是欧氏距离或余弦相似性）， $m$ 是一个边界，通常称为边距, 它确定了多远的距离被认为是足够的以区分不同类别的样本。这些函数是训练过程中的一部分, 通过调整模型的参数最小化这些损失, 模型学会将相似的样本拉近, 将不相似的样本推远。

**工作流程如下：**

1. 用户上传一张查询图片（Query Image）到系统。
2. 通过视觉搜索API，这张图片被发送到服务器。
3. 图像特征提取模块处理查询图片，提取重要的视觉特征。
4. 相似性搜索模块接收这些特征并与搜索索引中存储的图像特征进行比较。
5. 搜索结果基于相似性排序，并返回给用户。

**系统要求：**

- 所有服务器需要被监控以确保它们正常运行。
- 如果有服务器健康状况不佳，应该替换为新服务器。
- 当系统负载变高时，应该增加服务器数量或分配更多计算资源。

Lec



- **Dynamic Memory Model**：它负责管理记忆示例（Memory Exemplars），以便在不同任务之间保持知识的连贯性。它可能涉及到选择有代表性的样本来保留以往任务的知识。
- **Adaptive Working Model**：这是模型的一个活跃部分，它在新任务上进行训练，并适应新的数据。

这个过程包括几个关键步骤：

- **Knowledge Rehearsal**：这通常意味着在训练新任务时，模型会复习旧的任务样本，以避免遗忘。
- **Knowledge Refreshing**：这可能涉及更新模型对旧任务的理解，以纳入新的知识。
- **Knowledge Consolidation**：这是指固化新旧知识，使模型能够有效地在多个任务上运行。

底部的两个方程表示损失函数：

- **$L^m$**：可能代表用于训练动态记忆模型的损失函数，它由三个部分组成：校准损失（L^m_cali），交叉熵损失（L^m_ce）和三元组损失（L^m_trip）。
- **$L^w$**：代表用于训练自适应工作模型的损失函数，由对抗性损失（$L^w_{anti}$）、交叉熵损失（L^w_ce）和三元组损失（L^w_trip）组成。

Knowledge Rehersal 

1. 任务 $\mathrm{t}$ : 系统接收到一批新样本 $D_t^{t r a i n}$ 和一组记忆示例。这些数据被用来训练动态记忆模型, 其中可能会执行知识复习以及知识刷新, 以整合新信息并巩固已有知识。
2. 任务 $\mathbf{t + 1}$ : 随着系统进展到下一个任务, 它再次接收到新样本 $D_{t+1}^{t r a i n}$ 和更新后的记忆示例集。这个新任务的数据将会用于进一步训练模型, 同样涉及知识复习和刷新。

#### (a) Inter-Person Similarity Context

这部分展示了不同人物之间的相似性如何被模型考虑。这种上下文信息有助于模型理解和区分在监控视频中出现的不同个体。

#### (b) Intra-Person Attribute Context

这部分关注单个人物的各个属性之间的内在联系。输入图像通过一系列的LSTM（长短期记忆网络）单元进行编码，每个单元负责处理图像的一个时间步长的信息。通过注意力机制，模型学习到哪些属性在决定最终的上下文向量`z*`时最重要。

#### (c) Inter-Attribute Correlation

在这个部分，编码得到的上下文向量被用来预测人物的属性。另一系列的LSTM单元被用于这一解码阶段，每个单元负责一个特定的属性，例如性别、年龄、服装类型等。这些LSTM单元的作用是识别属性之间的相互依赖性。

#### (a) Embed

这一部分表现了原始特征的嵌入过程，即将实际特征（红色箭头）和合成特征（蓝色箭头）映射到一个共同的空间（绿色区域），这可以是语义空间或视觉空间。这一步骤是为了确保不同类型的特征可以在同一个空间内进行比较和处理。

#### (b) Embed + adversarial

在这一部分，GAN被引入作为对抗性的组件。在此情境下，GAN通常包含两个部分：一个生成器和一个判别器，生成器尝试生成无法与真实数据区分的合成特征，而判别器尝试区分真实特征和合成特征。这一步的目的是通过对抗性的过程来提高嵌入特征的质量。

#### (c) SAL

最终部分，共生对抗学习（SAL）结合了两个GANs：合成GAN（Synthesis-GAN）和对齐GAN（Alignment-GAN）。合成GAN旨在生成难以区分的特征，以增强模型的泛化能力。对齐GAN则确保了不同特征间的一致性和对齐。这两个GAN在一定程度上相互支持，合作提高整个模型的表现。共生性体现在它们共同优化和增强模型能力。

**（a）Embed（嵌入）**：

- 这个图的左侧部分（a）似乎展示了一个数据特征的嵌入过程，其中有“真实特征”和“合成特征”两种。
- “真实特征”和“合成特征”被嵌入到了一个共同的空间中，这个空间被分成了“语义空间”和“视觉空间”。

**（b）Embed + adversarial（嵌入+对抗）**：

- 中间的图（b）引入了GAN，这通常包含一个生成器和一个鉴别器。生成器试图生成越来越逼真的数据，而鉴别器则试图区分真实数据和生成的数据。
- 这个过程可以提升合成特征的质量，使它们在共同空间中与真实特征更难以区分。

**（c）SAL（共生对抗性学习）**：

- 右侧的图（c）展示了一个更高级的框架，其中包含“Synthesis-GAN”和“Alignment-GAN”。
- 这里“Synthesis-GAN”用于生成合成特征，而“Alignment-GAN”则用于调整这些特征，使得它们更好地与真实特征对齐。
- 两者相互作用：Synthesis-GAN的产出被用于提高Alignment-GAN的性能，而Alignment-GAN则反过来增强Synthesis-GAN的生成能力。这种相互作用被称作“共生对抗”。
- 除此之外，还有“Regularize（规范化）”和“Augment（增强）”的概念被引入，这些步骤可能用于改善模型的泛化能力和处理更多样化的数据。

具体来说，这个过程开始于一个查询图像（Query），这是系统要找到的目标个体的图像。系统首先进行区域感兴趣池化（Region of Interest, ROI pooling），将查询图像通过“Res50 Part1”进行特征提取，这是基于ResNet-50模型的前几层。然后，这些特征用于指导神经搜索网络的搜索过程。

神经搜索网络在一系列迭代步骤中缩小搜索范围，每一步都利用ROI池化和“Res50 Part1”提取画廊图像（Gallery Image）中的特征，然后进一步缩小搜索区域。这些步骤通过红色框表示出来，每一步都更加集中于候选区域。在最后一步，一个更深层的特征提取网络“Res50 Part2”（对应ResNet-50的后几层）被用来提取更复杂的特征，以实现最终的个人识别。
